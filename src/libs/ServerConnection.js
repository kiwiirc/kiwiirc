 ...
 */
export function createChannelConstructor(_addr, sessionId, _socketChannel, socksOptions) {
    let addr = _addr.toLowerCase();

    if (!serverConnections[addr]) {
        serverConnections[addr] = createNewConnection(addr, sessionId, socksOptions);
    }
 ...
 */
function createNewConnection(wsAddr, sessionId, socksOptions) {
    ...
    connection.connect = function connect() {
        ...
        connection.ws = new SockJS(wsAddr, null, socksOptions);
