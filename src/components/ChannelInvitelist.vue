    ...
    <div class="kiwi-channelinvitelist">
        <form class="u-form kiwi-channelinvitelist" @submit.prevent="">
            ...
            <div v-if="supportsAccounts">
                ...
                <table v-if="inviteListAccounts.length > 0" class="kiwi-channelinvitelist-table">
                    <tr>
                    ...
                    <tr v-for="invite in inviteListAccounts" :key="invite.invited">
                        <td class="kiwi-channelinvitelist-table-mask">{{ displayMask(invite) }}</td>
            ...
            <div v-if="supportsAccounts && inviteListNonAccounts.length > 0">
                ...
                <table class="kiwi-channelinvitelist-table">
                    <tr>
                        <th>{{ $t('invites_user') }}</th>
                        <th>{{ $t('invites_by') }}</th>
                        <th/>
...
export default {
    ...
    computed: {
        ...
        knownAccounts() {
            ...
            let accountUsers = [];
            for (let i in users) {
                if (users[i].account && inviteAccountNames.indexOf(users[i].account) === -1) {
                    accountUsers.push(users[i]);
                }
            }
